...
INLINE Bitboard attacks_by(const Position *pos, const PieceType pt, Color c)
{
  if (pt == PAWN)
    return pawn_attacks_bb(pieces_cp(c, PAWN), c);
  else
  {
    Bitboard threats = 0;
...
INLINE Bitboard check_squares(const Position *pos, const PieceType pt)
{
  return pos->st->checkSquares[pt];
}
