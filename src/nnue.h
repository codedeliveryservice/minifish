...
#define EvalFile "HL64-qa101-qb160-S2-T77novT79maraprmay.rearranged.8-bit.nnue"
...
struct NNUE {
  _Alignas(SIMD_ALIGNMENT) int16_t featureTransformerWeights[2][6][64][HIDDEN_WIDTH];
  _Alignas(SIMD_ALIGNMENT) int16_t featureTransformerBiases[HIDDEN_WIDTH];
  _Alignas(SIMD_ALIGNMENT) int16_t outputWeights[2 * HIDDEN_WIDTH];
                           int16_t outputBias;
};
...
struct NNUEAccumulator {
  union {
    _Alignas(SIMD_ALIGNMENT) int16_t colors[COLOR_NB][HIDDEN_WIDTH];
    _Alignas(SIMD_ALIGNMENT) int16_t both[COLOR_NB * HIDDEN_WIDTH];
  };
